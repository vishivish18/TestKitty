    ...
    ***************************************************************************/
    onConnect: function (session, socket) {
        ...
        try {
            ...
            if (session.roomid != "undefined" && session.roomid != null) {

                UserInRoom.findOne(session.roomid, function userCreated(err, user) {
                    if (user != null && user != "undefined") {
                        if (user.Status == null) {
                            user.Status = "D";
                            user.save();
                            sails.sockets.emit(socketId, 'hello', { hello: 'world' });
                            console.log(user);
                            UserInRoom.subscribe(socket, user, 'message');
                            UserInRoom.watch(socket)
                            UserInRoom.publishCreate(user, socket);
                        }
                    }
            ...
            console.log("Error in OnConnecting: ", e);
        }
    ...
    ***************************************************************************/
    onDisconnect: function (session, socket) {
        ...
        try {
            ...
            //            UserInRoom.findOne(session.roomid, function foundUser(err, user) {
            //                if (!err) {
            //                    UserInRoom.destroy(session.roomid, function userDestroyed(err) {
            //                        UserInRoom.publishDestroy(user.id, null, { previous: user });
            //                    });
            //                }
            //            });
